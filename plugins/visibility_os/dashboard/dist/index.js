(function () {
  ...
  function VisibilityOS() {
    ...
    function runOpportunityLane(item, endpoint) {
      const opportunityId = item.opportunity_id || item.id;
      post('/api/plugins/visibility-os/opportunities/' + opportunityId + endpoint, { actor: 'human' });
    }
    function fixCI(item) {
      runOpportunityLane(item, '/fix-ci');
    }
    function fixIssue(item) {
      runOpportunityLane(item, '/fix-issue');
    }
          ...
          return h(Card, { key: item.id, className: 'border-current/15' },
            ...
            h(CardContent, { className: 'space-y-2' },
              ...
              h('div', { className: 'flex gap-2 flex-wrap pt-2' },
                ...
                item.can_fix_pr_ci && h(ActionButton, { disabled: busy, onClick: function () { runOpportunityLane(item, '/fix-pr-ci'); }, className: 'border-emerald-500/50' }, 'Fix PR CI'),
                item.can_fix_docs && h(ActionButton, { disabled: busy, onClick: function () { fixIssue(item); }, className: 'border-emerald-500/50' }, 'Fix Docs'),
                item.can_fix_bug && h(ActionButton, { disabled: busy, onClick: function () { fixIssue(item); }, className: 'border-emerald-500/50' }, 'Fix Bug'),
                item.can_deflake_test && h(ActionButton, { disabled: busy, onClick: function () { fixIssue(item); }, className: 'border-emerald-500/50' }, 'Deflake Test'),
                item.can_prepare_handoff && h(ActionButton, { disabled: busy, onClick: function () { runOpportunityLane(item, '/prepare-handoff'); }, className: 'border-emerald-500/50' }, 'Prepare Handoff Branch'),
                item.can_review_pr && h(ActionButton, { disabled: busy, onClick: function () { runOpportunityLane(item, '/review-pr'); }, className: 'border-emerald-500/50' }, 'Review PR'),
                (!item.can_diagnose_ci && !item.can_fix_pr_ci && !item.can_fix_docs && !item.can_fix_bug && !item.can_deflake_test && !item.can_prepare_handoff && !item.can_review_pr && item.can_fix_issue) && h(ActionButton, { disabled: busy, onClick: function () { fixIssue(item); }, className: 'border-emerald-500/50' }, 'Fix Issue'),
